...
<div class="d-flex align-items-center gap-3 mb-4">
    ...
    <div>
        <h1 class="section-heading"><i class="bi bi-pencil"></i> Editar Editora</h1>
        <p class="section-sub">{{ $editora->nome }}</p>
    </div>
</div>
<div class="lib-card" style="max-width:540px;">
    ...
    <div class="p-4">
        <form action="{{ route('editoras.update', $editora->id) }}" method="POST">
            @csrf @method('PUT')
            <div class="mb-4"><label class="lib-label">Nome</label><input type="text" name="nome" class="lib-input" value="{{ old('nome', $editora->nome) }}" required></div>
            <div class="mb-4"><label class="lib-label">Cidade</label><input type="text" name="cidade" class="lib-input" value="{{ old('cidade', $editora->cidade) }}" required></div>
            <div class="mb-5"><label class="lib-label">Ano de Fundação</label><input type="number" name="ano_fundacao" class="lib-input" value="{{ old('ano_fundacao', $editora->ano_fundacao) }}" min="1000" max="{{ date('Y') }}" required></div>
